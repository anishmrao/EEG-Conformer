...
class ExP():
    def __init__(self, train_subs, val_sub, test_sub, mdl=False, mdl_ratio=0.5):
        super(ExP, self).__init__()
        self.batch_size = 72
        self.n_epochs = 5
        self.n_finetune_epochs = 3
        self.c_dim = 4
        self.lr = 0.0002
        self.b1 = 0.5
        self.b2 = 0.999
        ...
        self.start_epoch = 0
        self.root = '/home/msai/anishmad001/codes/EEG-Conformer/data/'

        randomFolder = str(time.strftime("%Y-%m-%d--%H-%M", time.localtime()))+ '-'+str(random.randint(1,1000))

        if(mdl):
            self.log_write_file = os.path.join("./results", randomFolder, "log_subject%d_mdl.txt" % self.test_sub)
        else:
            self.log_write_file = "./results/log_subject%d.txt" % self.test_sub
        ...
        self.model_save_path = '/home/msai/anishmad001/codes/EEG-Conformer/models/'
    ...
    def get_source_data(self, sub):
        # train data
        total_data = scipy.io.loadmat(self.root + 'A0%dT.mat' % sub)
        ...
        # test data
        test_tmp = scipy.io.loadmat(self.root + 'A0%dE.mat' % sub)
    ...
    def get_train_val_test_data(self):
        ...
        self.valData, self.valLabel, testData, testLabel = self.get_source_data(self.val_sub)
        ...
        self.finetuneData, self.finetuneLabel, self.unseenTestData, self.unseenTestLabel = self.get_source_data(self.test_sub)
    ...
    def train(self, mdl=False):
        ...
        if(mdl):
            torch.save(self.model.module.state_dict(), os.path.join(model_path, 'model_mdl.pth'))
        else:
            torch.save(self.model.module.state_dict(), os.path.join(model_path, 'model.pth'))
        ...
        if(self.load_best_model):
            self.model = self.model.module.load_state_dict(torch.load(best_model_path))
        ...
        return unseen_acc, test_acc, bestAcc, averAcc
        # writer.close()
    ...
    def finetune(self):
        img = self.finetuneData
        label = self.finetuneLabel
        test_data = self.unseenTestData
        test_label = self.unseenTestLabel
        ...
        for e in range(self.n_finetune_epochs):
            ...
            if (e + 1) % 1 == 0:
                ...
                if acc > bestAcc:
                    bestAcc = acc
                    torch.save(self.model.module.state_dict(), os.path.join(model_path, 'model_finetuned_best_acc.pth'))
                    # Y_true = test_label
                    # Y_pred = y_pred
        
        torch.save(self.model.module.state_dict(), os.path.join(model_path, 'model_finetuned.pth'))
...
def getLosoSplit(subs, val_sub=None, test_sub=None):
    if(not val_sub):
        val_sub = random.choice(subs)

    if(not test_sub):
        test_sub = random.choice(subs)
        while(test_sub == val_sub):
            test_sub = random.choice(subs)

    train_subs = [x for x in subs if x!=test_sub and x!=val_sub]
...
def main():
    ...
    prev_test = None
    skip_list = [3,4,5,6,7,8,9]
    num = 0
    mdl = False

    if(not mdl):
        result_write_file = "./results/sub_result.txt"
    else:
        result_write_file = "./results/sub_result_mdl.txt"
    ...
    for i, sub in enumerate(subs):
        ...
        exp = ExP(train_subs, val_sub, test_sub, mdl=mdl)
        if(i == 0):
            summary(exp.model, (1, 22, 1000))
        print("Number of trainable parameters:", count_parameters(exp.model))

        unseen_acc, test_acc, bestAcc, averAcc = exp.train(mdl)

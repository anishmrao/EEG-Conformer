...
class ExP():
    def __init__(self, train_subs, val_subs, test_subs, randomFolder, fold_num, patience=50, mdl=False, mdl_ratio=0.5):
        super(ExP, self).__init__()
        self.batch_size = 8
        self.n_epochs = 500
        self.n_finetune_epochs = 50
        self.c_dim = 4
        self.lr = 0.004 
    ...
    def train(self, mdl=False):
        ...
        self.optimizer = torch.optim.SGD(params=self.model.parameters(), lr=self.lr, weight_decay=self.weight_decay, momentum=self.momentum)
...
def main():
    ...
    for i in range(len(folds)):
        ...
        exp = ExP(train_subs, val_subs, test_subs, randomFolder, fold_num=i+1, mdl=mdl)
        if(i == 0):
